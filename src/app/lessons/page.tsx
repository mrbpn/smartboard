...
import { timeAgo, subjectColor, SUBJECTS } from "@/lib/utils";
...
export default function LessonsPage() {
  ...
  return (
    <div className="p-8">
      <PageHeader
        title="Lessons"
        subtitle={`${MOCK_LESSONS.length} lessons in your library`}
        action={
                ...
                <div className="flex items-start justify-between gap-2 mb-3">
                  <div className="flex-1 min-w-0">
                    <Link href={`/lessons/${lesson.id}`}>
                      <h3 className="font-display text-lg text-ink-800 leading-tight hover:text-sage-700 transition-colors line-clamp-2">
                        {lesson.title}
                      </h3>
                    </Link>
                  ...
                  <div className="relative flex-shrink-0">
                    ...
                    {openMenu === lesson.id && (
                      <div className="absolute right-0 top-8 bg-white border border-ink-200 rounded-xl shadow-lg z-10 w-40 overflow-hidden animate-fade-in">
                        ...
                        <button
                          onClick={() => { setDeleteTarget(lesson); setOpenMenu(null); }}
      ...
      <Modal open={!!deleteTarget} onClose={() => setDeleteTarget(null)} title="Delete lesson" size="sm">
        ...
        <div className="flex gap-2 justify-end">
          ...
          <Button variant="danger" onClick={() => setDeleteTarget(null)}>Delete</Button>
        </div>
      </Modal>
    </div>
  );
